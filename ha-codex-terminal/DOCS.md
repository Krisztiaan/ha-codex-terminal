...
- Persists Codex state in `CODEX_HOME=/data/codex_home` (`auth.json`, `config.toml`, `AGENTS.md`). `config.toml` and `AGENTS.md` can be managed through the add-on options UI; `auth.json` can also be uploaded safely from the Web UI and is then refreshed in place by Codex.
...
- Bundles a dedicated Home Assistant Python environment (`ha-python`, `ha-pip`, `hass`) pinned to `homeassistant==2026.3.1`, built on a managed Python `3.14.2`, which brings in the matching Home Assistant dependency stack (`aiohttp`, `httpx`, `SQLAlchemy`, `PyYAML`, `Jinja2`, `websockets`, etc.).
...
- Keeps a dedicated tmux session running even when ingress is closed so Codex continues working between browser attaches. Shell history is stored at `/data/.codex_bash_history` (capped).
...
The options page accepts optional multiline fields:

| Option key        | Description                                                                 | Target file (persisted in `CODEX_HOME`) |
|-------------------|-----------------------------------------------------------------------------|------------------------------------------|
| `auth_json_seed`  | One-time seed for `auth.json` (written only if missing). Prefer the Web UI upload. | `/data/codex_home/auth.json`      |
| `auth_json`       | Deprecated alias for `auth_json_seed` (kept for back-compat).              | `/data/codex_home/auth.json`             |
| `config_toml`     | Codex preferences (`~/.codex/config.toml`). When set, overwrites `CODEX_HOME/config.toml` on start. | `/data/codex_home/config.toml` |
| `agents_md`       | Persistent memory/agent instructions (`~/.codex/AGENTS.md`). When set, overwrites `CODEX_HOME/AGENTS.md` on start. | `/data/codex_home/AGENTS.md` |
...
If `config_toml` is blank, the add-on seeds `CODEX_HOME/config.toml` with recommended defaults on first run. It does not auto-update an existing file; edit `/data/codex_home/config.toml` directly if you want to manage it in-terminal.

If `agents_md` is blank, the add-on seeds `CODEX_HOME/AGENTS.md` with a concise environment brief covering available tools, mounts, and guardrails. Supply your own text in the option field to override it.

Changes are written on add-on start (and after saving options) with `chmod 600` permissions. `CODEX_HOME` is also available as `/root/.codex` via symlink for the Codex CLI. Use the Web UI `Auth.json` action to upload or remove credentials without sending them through the terminal session.
...
2. If required, use the `Auth.json` action in the Web UI to upload credentials safely, or use the legacy `auth_json_seed`/`auth_json` option only for one-time seeding.
3. Disable protection mode (`Settings → Add-ons → HA Codex Terminal → Configuration`).
4. Start the add-on and open the Web UI (ingress).
5. The ingress terminal attaches to the long-running `codex` tmux session (landing in the `main` window rooted at `/homeassistant` by default).
...
Codex is preinstalled globally during image build, and each session launches the pinned CLI directly. The image currently pins `@openai/codex@0.114.0`, `bash-language-server@5.6.0`, `ttyd 1.7.7`, Home Assistant CLI `4.46.0`, and a dedicated Python venv with `homeassistant==2026.3.1` on Python `3.14.2`. Tool caches default to `/tmp` (`/tmp/.npm`, `/tmp/.cache`, `/tmp/.local`, `/tmp/.npm-global`), keeping snapshots lean.
...
- Prefer the Web UI `Auth.json` upload/remove flow over the `auth_json_seed`/`auth_json` options so credentials are not stored in the Supervisor options database.
